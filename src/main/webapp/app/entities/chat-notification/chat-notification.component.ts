...
export class ChatNotificationComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.chatNotificationService
      ...
      .subscribe((res: HttpResponse<IChatNotification[]>) => this.paginateChatNotifications(res.body, res.headers));
  }
  ...
  transition() {
    this.router.navigate(['/chat-notification'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  protected paginateChatNotifications(data: IChatNotification[], headers: HttpHeaders) {
